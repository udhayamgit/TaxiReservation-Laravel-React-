...
class DispatcherPanel extends React.Component {
    ...
    handleRequestShow(trip) {
        ...
        if(trip.current_provider_id == 0 && trip.broadcast == 'NO') {
            this.setState({
                        ...
                        <a className="text-black" href="#">
                            <div className="media">
                                <div className="media-body">
                                    <p className="mb-0-5">{trip.user ? trip.user.first_name : ''} {trip.user ? trip.user.last_name : ''} 
            ...
            return (
                    <div className="il-item" key={trip.id} onClick={this.handleClick.bind(this, trip)}>
                        ...
                        <a className="text-black" href="#">
                            <div className="media"> 
                                <div className="media-body">
                                    <p className="mb-0-5"> Schedule Time  
                                        <span className="tag tag-success pull-right"> {trip.schedule_at} </span> 
                                    </p>
                                    <p className="mb-0-5">{trip.user ? trip.user.first_name : ''} {trip.user ? trip.user.last_name : ''} 
                                    {trip.status == 'COMPLETED' ?
                                        <span className="tag tag-success pull-right"> {trip.status} </span>
                                    ...
                                    : trip.status == 'SCHEDULED' ?
                                        <span className="tag tag-primary pull-right"> {trip.status} </span>
                                    : trip.status == 'SCHEDULES' ?
                                        <span className="tag tag-primary pull-right"> SCHEDULED </span>
                                    :
                                        <span className="tag tag-info pull-right"> {trip.status} </span>
                                    }
                                    </p>
        ...
        return (
            <div className="card card-block" id="create-ride">
                ...
                <form id="form-create-ride" onSubmit={this.createRide.bind(this)} method="POST">
                    ...
                    {this.state.check_estimate == true ?
                    <UserEstimateFare estimateview={this.state.estimate}/>
                    : "" }
                    <div className="row">
                        ...
                        <div className="col-xs-4">
                            <button className="btn btn-lg btn-success btn-block waves-effect waves-light" onClick={this.getEstimate.bind(this)}>
                                Estimate
                            </button>
                        </div>
                        <div className="col-xs-4">
...
class DispatcherAssignList extends React.Component {
    ...
    componentDidMount() {
        ...
        $.get('/estimated/fare', { 
                 s_latitude: this.props.trip.s_latitude,
                s_longitude: this.props.trip.s_longitude,
                d_latitude: this.props.trip.d_latitude,
                d_longitude: this.props.trip.d_longitude,
                service_type: this.props.trip.service_type_id
        }, function(response) { 
    ...
    render() {
        ...
        console.log('DispatcherAssignList - render', this.state.estimate);
        return (
            <div className="card">
                 <div className="card-header text-uppercase"><b>Estimate Trip Details</b></div>
                   {this.state.estimate ?
                  <UserEstimateFare estimateview={this.state.estimate} trip={this.props.trip} />
                  : ""}
...
class UserEstimateFare extends React.Component {
    ...
    resetTrip() {
       window.location.replace("/admin/dispatcher");
    }
     handleClick() {
         window.location.replace("/admin/dispatcher/cancel?request_id="+this.props.trip.id);
    }
    ...
    render() {
        ...
        //console.log('dsfds - render', this.props.estimateview);
        return (
                   <div style={style}>
                   <table className="table table-bordered">
                    <tbody>
                      <tr>
                        ...
                        <td>{this.props.estimateview.distance} Kms</td>
                      </tr>
                      <tr>
                         <th>ETA</th> 
                        <td>{this.props.estimateview.time}</td>
                      </tr>
                      <tr>
                         <th>Estimated Fare</th> 
                        <td>LBP{this.props.estimateview.estimated_fare}</td>
                      </tr>
                    </tbody>
                  </table>
                    <div className="row">
...
class ServiceTypes extends React.Component {
    render() {
        ...
        console.log('ServiceTypes', this.props.data.services);


        var mySelectOptions = function(result) {
            return <ServiceTypesOption key={result.id} id={result.id} name={result.name} />
        };
        return (
                <select name="service_type" id="service_type" className="form-control">
                    { (this.props.data.services) ?
                        this.props.data.services.map(function(result) {
                            return <ServiceTypesOption key={result.id} id={result.id} name={result.name} />
                        }) : ''
                    }
                </select>
